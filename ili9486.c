	...
	u8 x1, u8 y1)
{
	...
	st7735s_write_command(lcd, ILI9486_CASET);
	st7735s_write_data(lcd, data, sizeof(data));

	// row address set
	st7735s_write_command(lcd, ILI9486_RASET);
	data[1] = y0 + ILI9486_YSTART;
	data[3] = y1 + ILI9486_YSTART;
	st7735s_write_data(lcd, data, sizeof(data));

	// write to RAM
	st7735s_write_command(lcd, ILI9486_RAMWR);
}
...
static void st7735s_update_screen(struct st7735s *lcd)
{
	...
	st7735s_set_address_window(lcd, 0, 0, ILI9486_WIDTH - 1,
						ILI9486_HEIGHT - 1);
...
static int st7735sfb_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		st7735s_write_command(lcd, ILI9486_DISPOFF);
		break;
	case FB_BLANK_UNBLANK:
		st7735s_write_command(lcd, ILI9486_DISPON);
		break;
	}
...
static int st7735s_probe(struct spi_device *spi)
{
	...
	strcpy(spi->modalias, DEVICE_NAME);
